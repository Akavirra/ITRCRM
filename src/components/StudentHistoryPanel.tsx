...
export default function StudentHistoryPanel({ studentId }: StudentHistoryPanelProps) {
  const [recentHistory, setRecentHistory] = useState<StudentHistoryEntry[]>([]);
  const [allHistory, setAllHistory] = useState<StudentHistoryEntry[]>([]);
  ...
  const fetchHistory = useCallback(async (isInitial = false) => {
    try {
      ...
      const recentRes = await fetch(`/api/students/${studentId}/history?recent=true&limit=4`);
      const recentData = await recentRes.json();
      setRecentHistory(recentData.history || []);

      // Fetch all history
      const allRes = await fetch(`/api/students/${studentId}/history`);
