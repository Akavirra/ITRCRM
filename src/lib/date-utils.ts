...
function parseDatabaseDate(dateInput: string | Date | null | undefined): Date {
  ...
  // IMPORTANT: This format from Neon PostgreSQL is already in UTC, the offset indicates the original timezone
  // We should parse it as UTC directly, not apply timezone conversion
  const offsetMatch = dateStr.match(/(\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2})([+-])(\d{2})(:\d{2})?$/);
  if (offsetMatch) {
    const [, dateTime] = offsetMatch;
    
    // Parse the datetime part as UTC directly (ignore the timezone offset)
