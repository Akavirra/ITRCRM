 ...
 *
 * Архітектура:
 *   - Логін: код учня (R0042) + PIN (6 цифр) + trust: boolean
 *   - Rate limit: окремо по IP (10/15хв) та по коду (5/15хв)
 *   - Lockout: після 5 невдалих спроб код блокується на 15 хв
 *   - Сесія: два режими, обираються при логіні через `trust`:
 *       • short / shared device  (trust=false, дефолт): TTL 2 год, idle-logout 60 хв
 *       • persistent / trusted    (trust=true):         TTL 30 днів, без idle-logout
 *     На trusted last_seen_at оновлюється при heartbeat і expires_at автопродовжується.
 *     На short — heartbeat оновлює last_seen_at, idle-логіка серверна (getStudentSession
 *     повертає null, якщо last_seen_at < now − 60хв).
 */
...
/** TTL короткої сесії (shared device, trust=false). */
export const SESSION_TTL_SHORT_MS = 2 * 60 * 60 * 1000; // 2 години
/** TTL довгої сесії (trusted device, trust=true). */
export const SESSION_TTL_LONG_MS = 30 * 24 * 60 * 60 * 1000; // 30 днів
/** Idle-timeout для short-сесій. Якщо last_seen_at старіший — сесія невалідна. */
export const SESSION_IDLE_TIMEOUT_MS = 60 * 60 * 1000; // 60 хв
/** Поріг для авто-refresh persistent-сесій (поки залишилось менше — продовжуємо). */
...
/** Cookie maxAge у секундах для short/long режимів. */
export function cookieMaxAgeSec(persistent: boolean): number {
  return Math.floor((persistent ? SESSION_TTL_LONG_MS : SESSION_TTL_SHORT_MS) / 1000);
}
...
export async function loginStudent(
  ...
  userAgent: string,
  trust: boolean = false,
): Promise<StudentAuthResult & { isPersistent: boolean }> {
  ...
  // Створюємо сесію — TTL обираємо за trust-параметром
  const sessionId = uuidv4();
  const ttlMs = trust ? SESSION_TTL_LONG_MS : SESSION_TTL_SHORT_MS;
  const expiresAt = new Date(Date.now() + ttlMs).toISOString();
  await studentRun(
    `INSERT INTO student_sessions (id, student_id, expires_at, ip, user_agent, is_persistent)
     VALUES ($1, $2, $3, $4, $5, $6)`,
    [sessionId, student.id, expiresAt, ip, (userAgent || '').slice(0, 500), trust]
  );
...
export async function getStudentSession(sessionId: string): Promise<StudentSessionRow | null> {
  ...
  const row = await studentGet<StudentSessionRow>(
    `SELECT id, student_id, created_at, last_seen_at, expires_at, is_persistent
  ...
  // Idle-перевірка для short-сесій: якщо last_seen_at старіший за SESSION_IDLE_TIMEOUT_MS,
  // трактуємо як expired (як ніби сесії немає). Persistent-сесії не перевіряються.
  if (!row.is_persistent) {
    const lastSeenMs = new Date(row.last_seen_at).getTime();
    if (Date.now() - lastSeenMs > SESSION_IDLE_TIMEOUT_MS) {
      // Прибираємо одразу, щоб не накопичувати tombstones.
      await studentRun(`DELETE FROM student_sessions WHERE id = $1`, [sessionId]).catch(() => {});
      return null;
    }
  }
...
/**
 * Оновлює last_seen_at + продовжує expires_at.
 * - short: expires = NOW + 2год (плаваюче вікно — поки активний, не вилогінить)
 * - persistent: expires = NOW + 30 днів (rolling)
 */
export async function refreshStudentSession(sessionId: string, isPersistent: boolean): Promise<string> {
  const ttlMs = isPersistent ? SESSION_TTL_LONG_MS : SESSION_TTL_SHORT_MS;
  const expiresAt = new Date(Date.now() + ttlMs).toISOString();
  await studentRun(
...
/** Лише оновити last_seen_at (для heartbeat) — без продовження expires_at. */
export async function touchStudentSession(sessionId: string): Promise<void> {
  await studentRun(
    `UPDATE student_sessions SET last_seen_at = NOW() WHERE id = $1`,
    [sessionId]
  );
}
...
/** Видаляє ВСІ сесії учня — для "Вийти на всіх пристроях". */
export async function deleteAllStudentSessions(studentId: number): Promise<void> {
  await studentRun(`DELETE FROM student_sessions WHERE student_id = $1`, [studentId]);
}
