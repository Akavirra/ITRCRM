...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const uploadResult = await uploadBuffer(buffer, 'completion-certificates', 'template');

    await run(
      `INSERT INTO system_settings (key, value, updated_at)
       VALUES ('completion_certificate_template_url', $1, NOW())
    ...
    console.error('Completion certificate template upload error:', error);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ url: res?.value || null });
  } catch (error: any) {
