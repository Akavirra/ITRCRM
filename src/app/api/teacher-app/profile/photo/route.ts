...
export async function POST(request: NextRequest) {
  ...
  const teacher = await getTeacher(verification.telegramId);
  ...
  try {
    ...
    if (teacher.photo_url) {
      const oldPublicId = getPublicIdFromUrl(teacher.photo_url);
...
export async function DELETE(request: NextRequest) {
  ...
  try {
    if (teacher.photo_url) {
      const publicId = getPublicIdFromUrl(teacher.photo_url);
